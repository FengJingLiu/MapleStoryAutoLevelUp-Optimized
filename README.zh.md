...
角色定位預設使用 YOLO，Hero 閾值可與怪物閾值分開設定：
...
可用 `.venv\Scripts\python.exe -m tools.evaluate_yolo_hero` 對 `screenshot/*_img_frame.png`
產生偵測 CSV 與聯絡表。
...
遠端模式中的掉線重新登入是限定用途的輸入例外。只有目前畫面偵測不到
MINIMAP 時才會進入重登入；進入後會停止所有遊戲操作並持續執行中文
RapidOCR，直到 MINIMAP 再次出現才恢復。頁面不必依固定順序出現，程式會
依目前識別到的畫面獨立執行：

1. 識別「與伺服器連線發生錯誤」後傳送 `Enter`。
2. 識別「連線」後傳送 `Enter`。
3. 識別 `4.漂漂豬` 且尚未出現頻道面板時，點擊 OCR 中心。
4. 識別未編號的「漂漂豬」頻道面板標記後，從 20 個頻道座標隨機選擇並雙擊。
5. 識別「開始遊戲」後點擊 OCR 中心。

每個動作仍需連續穩定 OCR 畫面確認，且同一頁保留重試冷卻。未知畫面、
OCR 漏識別、載入緩慢、頁面重複或輸入回覆不確定，都不會結束重登入；
程式沒有總逾時或最大嘗試次數，會一直安全掃描及重試。遊戲 UI 點擊使用
由 GC573 4K 擷取座標換算的 ESP32 絕對 HID 座標。此流程只恢復已驗證的
工作階段，不會輸入帳號密碼，也不處理驗證碼、雙重驗證或啟動器更新。
...
保持連通的線段，並在真正的繩索接觸點加入顯式箭頭尖端：

- 引導線與箭頭兩翼：RGB `(0, 127, 255)`，十六進位 `#007FFF`；若使用 OpenCV，BGR 為
  `(255, 127, 0)`。
- 箭頭尖端：在真正的繩索接觸點只放置 **1 個像素**，RGB `(0, 191, 255)`，
  十六進位 `#00BFFF`；若使用 OpenCV，BGR 為 `(255, 191, 0)`。尖端與引導線的
  曼哈頓距離不得超過 3 個小地圖像素。
- 箭頭兩翼可用普通引導線顏色繪製；程式只讀取亮藍色的單像素尖端。請使用
  1 像素鉛筆、100% 不透明度、關閉抗鋸齒，並儲存為 PNG，確保 RGB 完全一致。
- 顯式尖端一旦識別，繩索位置便固定為該像素，不受 Hero 位於引導線哪一側、
  怪物碰撞或失敗重試影響。沒有亮藍尖端的舊引導線仍使用「距離 Hero 較遠端」
  的相容判斷。
- 每條引導線必須是各自獨立的連通區域，不要把兩條繩索接在一起，也不要用
  其他路線顏色蓋住它；每條引導線只能綁定一個亮藍尖端。
